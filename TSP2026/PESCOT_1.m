function [f0s, alpha, pitchGrid, pitchDist, M] = PESCOT_1(y, epsilon, zeta, eta, beta, varargin)
%%%%
%   PESCOT-1 - Multi-Pitch Estimation with Sparse Clustering through Optimal
%   Transport
%
%   [f0s, alpha, M] = PESCOT_1(y, epsilon, zeta, eta, beta, nPitches,...
%   max_iter, pitchLim, maxHarm, nPitchGrid, freqGrid, doPrint)
%
%
%   INPUTS: 
...
nesterov = 0; % "nesterov acceleration", speeds up convergence
tol = 1.5e-2; % convergence tolerance

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Dictionary and cost matrix generation %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pitchGrid = linspace(pitchLim(1),pitchLim(2),nPitchGrid);
% if pitchLim(2)*maxHarm > pi
%     freqGrid = linspace(pitchLim(1),pi,nFreqsGrid);
% else
%     freqGrid = linspace(pitchLim(1),pitchLim(2)*maxHarm,nFreqsGrid);
% end
...
for iter = 1:max_iter
    ...
    % Gradient of LS term
    gradient = -A'*(y-A*v)/N;
    [alpha_new,dual0,dual1] = prox_operator_abs_l1(C,v-gamma*gradient,epsilon, zeta, eta, gamma,max_iter_prox_operator,dual0,dual1, beta);
